...
def dask_run():
    ...
    df = dd.read_csv("https://archive.ics.uci.edu/ml/machine-learning-databases/adult/adult.data")
    ...
    return(train_X, val_X, train_y, val_y) # This uses the single-machine scheduler by default
...
def pandas_rf(df):
    ...
    print(str(type(df)) + str(accuracy_score(val_y,predictions) * 100) + "% predicted correctly")
    end = time.time()
    print(str(type(df)) + " :took " + str(end - start))
    return()
...
def create_rf(train_X, val_X, train_y, val_y):
    ...
    predictions = model.predict(val_X)
    print(str(type(train_X)) + str(accuracy_score(val_y,predictions) * 100) + "% predicted correctly")
    end = time.time()
